...
{
	@Query(value="select * from manutencao m inner join os_impressora os on m.codigo = os.codigo "
			+ "where m.tipo_equipamento = 'impressora' and m.status_manutencao = 'ABERTO' order by(m.data_entrada) asc",
			nativeQuery=true)
	public List<OsImpressora> pesquisarOsImpressoraAberto();
